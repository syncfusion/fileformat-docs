...
gulp.task('ship-to-gitlab', function (done) {
    ...
    var gitPath = 'https://' + user + ':' + token + `@gitlab.syncfusion.com/content/fileformat-docs`;
    console.log('Clone has been started...!');
    var clone = shelljs.exec('git clone ' + gitPath + ' -b ' + branch + ' ' + `../../../gitlabRepo/fileformat-docs`, {
        silent: false
    });
    if (clone.code !== 0) {
        ...
        for (var changedFileName of changedFileNames.split(',')) {
            ...
            if (changedFileName !== null && changedFileName !== '' && changedFileName !== '.gitignore' && !changedFileName.includes('.gitlab')) {
                ...
                if (fs.existsSync('../fileformat-docs/' + changedFileName)) {
                    // It will update the modified files
                    if (fs.existsSync('../../../gitlabRepo/fileformat-docs/' + changedFileName)) {
                        shelljs.cp('-rf', `../fileformat-docs/` + changedFileName, `../../../gitlabRepo/fileformat-docs/` + changedFileName);
                    }
                    else {
                        // It will update the newly added files
                        if (fs.existsSync('../../../gitlabRepo/fileformat-docs/')) {
                            shelljs.cp('-rf', `../fileformat-docs/` + changedFileName, `../../../gitlabRepo/fileformat-docs/` + changedFileName);
                        }
                    }
                    ...
                    if (fs.existsSync('../../../gitlabRepo/fileformat-docs/' + changedFileName)) {
                        shelljs.rm('-rf', `../../../gitlabRepo/fileformat-docs/` + changedFileName);
        ...
        shelljs.cd(`../../../gitlabRepo/fileformat-docs`);
